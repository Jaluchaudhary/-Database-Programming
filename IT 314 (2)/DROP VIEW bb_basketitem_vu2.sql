...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('Recipient Name: ' || NVL(v_ship_name, 'No name found'));

 
  order_info_pkg.basket_info_pp(12, v_shopper_id, v_order_date);
  DBMS_OUTPUT.PUT_LINE('Shopper ID: ' || v_shopper_id);
...
CREATE OR REPLACE PACKAGE BODY order_info_pkg IS
  ...
  BEGIN
    ...
    -- Use the private function to get the ship name
    p_ship_name := ship_name_pf(p_basket);
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('Invalid basket ID');
  END basket_info_pp;
...
BEGIN
  ...
  order_info_pkg.basket_info_pp(12, v_shopper_id, v_order_date, v_ship_name);


  DBMS_OUTPUT.PUT_LINE('Shopper ID: ' || v_shopper_id);
  DBMS_OUTPUT.PUT_LINE('Order Date: ' || v_order_date);
  DBMS_OUTPUT.PUT_LINE('Shipped-To Name: ' || v_ship_name);
END;
/
...
CREATE OR REPLACE PACKAGE BODY login_pkg IS
  ...
  BEGIN
    ...
    SELECT 'Y', idshopper, SUBSTR(zipcode, 1, 3) 
      INTO v_return, v_shopper_id, v_zip_prefix
      FROM bb_shopper
  ...
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('Invalid logon values.');
      RETURN v_return; 
  END verify_logon_fn;
END login_pkg;
/
...
BEGIN
  ...
  v_result := login_pkg.verify_logon_fn('gma1', 'goofy');
  

  DBMS_OUTPUT.PUT_LINE('Logon : ' || v_result);
  DBMS_OUTPUT.PUT_LINE('Shopper ID: ' || login_pkg.v_shopper_id);
  DBMS_OUTPUT.PUT_LINE('Zip Code: ' || login_pkg.v_zip_prefix);
END;
/
...
CREATE OR REPLACE PACKAGE BODY SHOP_QUERY_PKG IS
  ...
  PROCEDURE GET_SHOPPER_INFO_BY_ID (P_SHOPPER_ID IN NUMBER) IS
    ...
    V_EMAIL       VARCHAR2(25);
  BEGIN
    -- Query using the correct column name 'LASTNAME'
    SELECT FIRSTNAME, LASTNAME, CITY, STATE, PHONE, EMAIL
    INTO V_FIRST_NAME, V_LAST_NAME, V_CITY, V_STATE, V_PHONE, V_EMAIL
    FROM BB_SHOPPER
  ...
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('No shopper found with the provided ID.');
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('An error occurred: ' || SQLERRM);
  END GET_SHOPPER_INFO_BY_ID;

  -- Procedure to get shopper info by last name
  PROCEDURE GET_SHOPPER_INFO_BY_LASTNAME (P_LAST_NAME IN VARCHAR2) IS
    V_FIRST_NAME  VARCHAR2(15);
    V_LAST_NAME   VARCHAR2(20);
    V_CITY        VARCHAR2(20);
    V_STATE       CHAR(2);
  ...
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('No shopper found with the provided last name.');
    WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE('An error occurred: ' || SQLERRM);
  END GET_SHOPPER_INFO_BY_LASTNAME;

END SHOP_QUERY_PKG;
/

BEGIN
  SHOP_QUERY_PKG.GET_SHOPPER_INFO_BY_ID(23);
END;
/
BEGIN
  SHOP_QUERY_PKG.GET_SHOPPER_INFO_BY_LASTNAME('Ratman');
END;
/
