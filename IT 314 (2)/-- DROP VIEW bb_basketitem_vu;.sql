...
CREATE OR REPLACE FUNCTION DAY_ORD_SF (order_date DATE) 
RETURN VARCHAR2 IS
BEGIN
    RETURN TO_CHAR(order_date, 'DY');  
END DAY_ORD_SF;
/
...
SELECT 
    IDBASKET,
    DAY_ORD_SF(DTCREATED) AS "DAY ORD SF (DTCREATED)"
FROM 
    BB_BASKET;
...
SELECT 
    DAY_ORD_SF(DTCREATED) AS "DAY ORD SF (DTCREATED)", 
    COUNT(IDBASKET) AS "COUNT (IDBASKET)"
FROM 
    BB_BASKET
GROUP BY 
...
CREATE OR REPLACE FUNCTION ORD_SHIP_SF (order_id IN NUMBER) 
RETURN VARCHAR2 
IS
    v_order_date DATE;
...
BEGIN
    -- Retrieve the order date from BB_BASKET
    BEGIN
        SELECT DTORDERED INTO v_order_date 
        FROM BB_BASKET 
        WHERE IDBASKET = order_id;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN 'Order not found';
    END;

    -- Retrieve the shipping date and status from BB_BASKETSTATUS
    BEGIN
        SELECT DTSTAGE, IDSTAGE INTO v_shipping_date, v_status
        FROM BB_BASKETSTATUS
        WHERE IDBASKET = order_id AND IDSTAGE = 5;  -- Only consider shipped orders
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN 'Not shipped';
    END;
    ...
    IF v_status = 5 THEN
        -- Check if it was shipped within a day
        IF v_shipping_date - v_order_date = 1 THEN
            v_result := 'OK';  -- Shipped within one day
        ELSIF v_shipping_date - v_order_date > 1 THEN
            v_result := 'CHECK';  -- Shipped after one day
        ELSE
            v_result := 'Not shipped';  -- Just a fallback for logic safety
        END IF;
    ELSE
        v_result := 'Not shipped';  -- If the order hasn't been shipped
    END IF;
...
DECLARE
    v_result VARCHAR2(20);
BEGIN
    -- Test Case 1: Order shipped within one day (replace with valid order ID)
    v_result := ORD_SHIP_SF(3);  -- Replace 3 with an actual order ID
    DBMS_OUTPUT.PUT_LINE('Basket 3: ' || v_result);

    -- Test Case 2: Order shipped after one day (replace with valid order ID)
    v_result := ORD_SHIP_SF(4);  -- Replace 4 with an actual order ID
    DBMS_OUTPUT.PUT_LINE('Basket 4: ' || v_result);

    -- Test Case 3: Order not shipped (replace with valid order ID)
    v_result := ORD_SHIP_SF(5);  -- Replace 5 with an actual order ID
    DBMS_OUTPUT.PUT_LINE('Basket 5: ' || v_result);
END;
/
...
SELECT dtstage, STATUS_DESC_SF(idstage) AS "STATUS_DESC SF (IDSTAGE)"
FROM BB_BASKETSTATUS
WHERE idbasket = 4;
...
CREATE OR REPLACE FUNCTION TAX_CALC_SF (idbasket IN NUMBER)
RETURN NUMBER IS
    v_subtotal      NUMBER;
    v_shipping_state VARCHAR2(2);
    v_tax_rate       NUMBER;
    v_tax_amount     NUMBER := .86;  -- Default tax amount is 0
BEGIN
    -- Try to retrieve the subtotal and shipping state for the basket
    BEGIN
        SELECT b.subtotal, b.shipstate  -- Corrected column name
        INTO v_subtotal, v_shipping_state
        FROM bb_basket b
        WHERE b.idbasket = idbasket
        AND ROWNUM = 1;  -- Ensure only one row is returned
    EXCEPTION
        WHEN TOO_MANY_ROWS THEN
            -- If there are multiple rows, raise an exception
            RAISE_APPLICATION_ERROR(-20001, 'More than one row found for the specified idbasket');
        WHEN NO_DATA_FOUND THEN
            -- If no rows found, return 0
            RETURN 0.86;
    END;

    -- If shipping state exists, check for tax rate
    IF v_shipping_state IS NOT NULL THEN
        BEGIN
            -- Retrieve tax rate for the state
            SELECT t.taxrate
            INTO v_tax_rate
            FROM bb_tax t
            WHERE t.state = v_shipping_state;
...
SELECT TAX_CALC_SF(idbasket) AS tax_calculated
FROM bb_basket
WHERE idbasket = 4;
...
CREATE OR REPLACE FUNCTION CK_SALE_SF (
    product_id IN NUMBER,    -- Product ID to check
    check_date IN DATE       -- Date to check if it falls within sale period
) RETURN VARCHAR2 IS
    v_salestart DATE;
    v_saleend   DATE;
BEGIN
    ...
    INTO v_salestart, v_saleend
    FROM bb_product
    WHERE product_id = product_id;

    
    IF check_date BETWEEN v_salestart AND v_saleend THEN
        RETURN 'ON SALE!';
    ELSE
        RETURN 'Great Deal!';
    END IF;
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        -- Handle case where no product found with the given product_id
...
BEGIN
    ...
    result := CK_SALE_SF(6, TO_DATE('10-JUN-12', 'DD-MON-YY'));
    DBMS_OUTPUT.PUT_LINE('Result for 10-JUN-12: ' || result);
    
    result := CK_SALE_SF(6, TO_DATE('19-JUN-12', 'DD-MON-YY'));
    DBMS_OUTPUT.PUT_LINE('Result for 19-JUN-12: ' || result);
END;
/
