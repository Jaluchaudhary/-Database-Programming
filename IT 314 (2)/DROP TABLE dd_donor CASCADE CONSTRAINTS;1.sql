...
CREATE OR REPLACE FUNCTION DD_MTHPAY_SF (
    p_monthly_payments NUMBER,
    p_pledge_amount NUMBER
) RETURN NUMBER IS
    v_monthly_payment NUMBER;
BEGIN
    
    v_monthly_payment := p_pledge_amount / p_monthly_payments;
...
BEGIN
    ...
    v_monthly_payment := DD_MTHPAY_SF(12, 240);
  
    DBMS_OUTPUT.PUT_LINE('Monthly Payment Amount: $' || TO_CHAR(v_monthly_payment));
END;
/



CREATE OR REPLACE FUNCTION DD_PROJTOT_SF(p_project_id NUMBER)
RETURN NUMBER
IS
    v_total NUMBER := 0; -- Initialize total to 0
BEGIN

    SELECT NVL(SUM(PLEDGEAMOUNT), 0) 
    INTO v_total
    FROM PLEDGES
...
ALTER FUNCTION DD_PROJTOT_SF COMPILE;
SHOW ERRORS FUNCTION DD_PROJTOT_SF;
...
BEGIN
    ...
    v_total := DD_PROJTOT_SF(1); 
    DBMS_OUTPUT.PUT_LINE('Total Pledge Amount: ' || TO_CHAR(v_total, '$9,999,999.99'));
END;
/
...
SELECT 
    ...
    TO_CHAR(DD_PROJTOT_SF(PROJECTID), '$9,999,999.99') AS PROJECT_PLEDGE_TOTAL
FROM 
    PROJECTS
ORDER BY 
    PROJECTID;
...
CREATE OR REPLACE FUNCTION DD_PLSTAT_SF (idStatus IN NUMBER)
RETURN VARCHAR2 IS
    statusDesc VARCHAR2(15);
BEGIN
    -- Map status ID to description
    CASE idStatus
        WHEN 10 THEN statusDesc := 'Open';
        WHEN 20 THEN statusDesc := 'Complete';
        WHEN 30 THEN statusDesc := 'Closed';
...
SELECT 
    idPledge, 
    Pledgedate, 
    DD_PLSTAT_SF(idStatus) AS PledgeStatus
FROM 
    DD_Pledge;
...
DECLARE
    statusDesc VARCHAR2(15);
BEGIN
    statusDesc := DD_PLSTAT_SF(10);
    DBMS_OUTPUT.PUT_LINE('Pledge Status: ' || statusDesc);
END;
/
