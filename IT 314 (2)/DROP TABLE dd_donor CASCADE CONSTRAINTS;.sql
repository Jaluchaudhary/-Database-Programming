...
DECLARE
    v_projname VARCHAR2(60);
    v_projgoal NUMBER(12,2);
    v_totalpledge NUMBER(12,2);
BEGIN
   
    SELECT Projname, Projfundgoal
    INTO v_projname, v_projgoal
    FROM DD_Project
    WHERE idProj = 501; 
    ...
    DBMS_OUTPUT.PUT_LINE('Project Goal: $' || v_projgoal);
    DBMS_OUTPUT.PUT_LINE('Total Pledged Amount: $' || NVL(v_totalpledge, 0));
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No project found with the specified ID.');
    WHEN OTHERS THEN
