...
BEGIN
    EXECUTE IMMEDIATE 'DROP TABLE DD_PAYTRACK';
EXCEPTION
    WHEN OTHERS THEN
...
-- Step 2: Create the DD_PAYTRACK table
CREATE TABLE DD_PAYTRACK (
    track_id NUMBER PRIMARY KEY,  -- Inline primary key definition
...
/*-- Step 3: Create the sequence DD_PTRACK_SEQ
CREATE SEQUENCE DD_PTRACK_SEQ
START WITH 1
INCREMENT BY 1
NOCACHE;
*/
-- Step 4: Create the trigger TRG_PAYTRACK
CREATE OR REPLACE TRIGGER trg_paytrack
AFTER INSERT OR UPDATE OR DELETE ON DD_Payment
FOR EACH ROW
DECLARE
    v_action VARCHAR2(10);
BEGIN
    -- Determine the type of action (INSERT, UPDATE, DELETE)
    IF INSERTING THEN
        v_action := 'INSERT';
        -- Insert tracking record for INSERT
        INSERT INTO DD_PAYTRACK (track_id, username, action_taken, action_date, idpay)
        VALUES (DD_PTRACK_SEQ.NEXTVAL, 'Patrick Lee', v_action, SYSDATE, :NEW.idPay);
    ELSIF UPDATING THEN
        v_action := 'UPDATE';
        -- Insert tracking record for UPDATE
        INSERT INTO DD_PAYTRACK (track_id, username, action_taken, action_date, idpay)
        VALUES (DD_PTRACK_SEQ.NEXTVAL, 'Patrick Lee', v_action, SYSDATE, :NEW.idPay);
    ELSIF DELETING THEN
        v_action := 'DELETE';
        -- Insert tracking record for DELETE
        INSERT INTO DD_PAYTRACK (track_id, username, action_taken, action_date, idpay)
        VALUES (DD_PTRACK_SEQ.NEXTVAL, 'Patrick Lee', v_action, SYSDATE, :OLD.idPay);
    END IF;
END;
/
...
VALUES (1001, 101, 500, SYSDATE, 'CC');
