...
CREATE OR REPLACE PROCEDURE STATUS_SHIP_SP (
    ...
    p_date_shipped IN DATE,      -- Date shipped
    p_shipper IN VARCHAR2,       -- Shipper name
    p_tracking_num IN VARCHAR2   -- Tracking number
)
AS
BEGIN
    -- Insert a new row into BB_BASKETSTATUS with stage 3 (shipped)
    INSERT INTO BB_BASKETSTATUS (
        IDBASKET, 
        IDSTATUS, 
        IDSTAGE, 
    ...
    VALUES (
        ...
        BB_STATUS_SEQ.NEXTVAL,           -- Get next ID from sequence
        3,                               -- IDSTAGE = 3 for shipped
        p_date_shipped,                  -- Date shipped
        'Shipped via ' || p_shipper,     -- Notes field: Shipper info
        p_tracking_num                   -- Tracking number
    );
    ...
    DBMS_OUTPUT.PUT_LINE('Shipment status updated for Basket # ' || p_basket_id);
END;
/


EXEC STATUS_SHIP_SP(3, TO_DATE('20-FEB-12', 'DD-MON-YY'), 'UPS', 'ZW2384YXK4957');
...
SELECT 
    ...
    IDBASKET, 
    IDSTAGE, 
    TO_CHAR(DTSTAGE, 'DD-MON-YY') AS DTSTAGE,  
    NOTES, 
    SHIPPINGNUM
FROM 
    BB_BASKETSTATUS
...
CREATE OR REPLACE PROCEDURE STATUS_SP (
    ...
    p_status_date OUT DATE            -- Output: Status date
)
AS
BEGIN
    -- Fetch the most recent status information for the specified basket ID
    SELECT 
        CASE 
            WHEN IDSTAGE = 1 THEN 'Submitted and received'
            WHEN IDSTAGE = 2 THEN 'Confirmed, processed, sent to shipping'
            WHEN IDSTAGE = 3 THEN 'Shipped'
            WHEN IDSTAGE = 4 THEN 'Cancelled'
            WHEN IDSTAGE = 5 THEN 'Back-ordered'
            ELSE 'Unknown status'
        END AS STATUS_DESC,
        DTSTAGE
    INTO 
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        p_status_desc := 'No status available for the specified basket ID.';
        p_status_date := NULL;
END;
/
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Basket ID 4: ' || v_status_desc || ', Date: ' || NVL(TO_CHAR(v_status_date, 'DD-MON-YYYY'), 'N/A'));
