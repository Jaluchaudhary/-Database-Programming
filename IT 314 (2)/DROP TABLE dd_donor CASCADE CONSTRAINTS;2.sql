...
DECLARE
    CURSOR pledge_cursor IS
        SELECT p.idPledge, d.Firstname, d.Lastname, p.Pledgedate, p.Pledgeamt, p.idProj, s.Statusdesc
        FROM DD_Pledge p
        JOIN DD_Donor d ON p.idDonor = d.idDonor
        JOIN DD_Status s ON p.idStatus = s.idStatus;
    
    v_idPledge DD_Pledge.idPledge%TYPE;
    v_Firstname DD_Donor.Firstname%TYPE;
    v_Lastname DD_Donor.Lastname%TYPE;
    v_Pledgedate DD_Pledge.Pledgedate%TYPE;
    v_Pledgeamt DD_Pledge.Pledgeamt%TYPE;
    v_idProj DD_Pledge.idProj%TYPE;
    v_Statusdesc DD_Status.Statusdesc%TYPE;
BEGIN
    ...
    LOOP
        ...
        FETCH pledge_cursor INTO v_idPledge, v_Firstname, v_Lastname, v_Pledgedate, v_Pledgeamt, v_idProj, v_Statusdesc;
        
  
        EXIT WHEN pledge_cursor%NOTFOUND;
        
       
        DBMS_OUTPUT.PUT_LINE('Pledge ID: ' || v_idPledge || 
                             ', Donor: ' || NVL(v_Firstname, 'N/A') || ' ' || NVL(v_Lastname, 'N/A') || 
                             ', Pledge Date: ' || TO_CHAR(v_Pledgedate, 'DD-MON-YYYY') || 
                             ', Amount: $' || v_Pledgeamt || 
                             ', Project ID: ' || v_idProj || 
                             ', Status: ' || v_Statusdesc);
    END LOOP;
