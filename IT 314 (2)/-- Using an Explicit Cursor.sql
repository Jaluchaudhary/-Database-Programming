...
DECLARE
  ...
  v_donor_id     dd_donor.idDonor%TYPE := 303; 
  v_pledge_amt   dd_pledge.Pledgeamt%TYPE;
  v_pledge_date  dd_pledge.Pledgedate%TYPE;
  v_project_name dd_project.Projname%TYPE;
  v_pay_amt      dd_payment.Payamt%TYPE;
  v_pay_date     dd_payment.Paydate%TYPE;
  v_pay_method   dd_payment.Paymethod%TYPE;

 
  CURSOR pledge_cursor IS
    SELECT p.Pledgeamt, p.Pledgedate, pr.Projname
      FROM dd_pledge p
      JOIN dd_project pr ON p.idProj = pr.idProj
      WHERE p.idDonor = v_donor_id;

  CURSOR payment_cursor IS
    SELECT pa.Payamt, pa.Paydate, pa.Paymethod
      FROM dd_payment pa
      JOIN dd_pledge pl ON pa.idPledge = pl.idPledge
      WHERE pl.idDonor = v_donor_id;

BEGIN
  
  DBMS_OUTPUT.PUT_LINE('Pledge Information for Donor ID: ' || v_donor_id);
  OPEN pledge_cursor;
  LOOP
    FETCH pledge_cursor INTO v_pledge_amt, v_pledge_date, v_project_name;
    EXIT WHEN pledge_cursor%NOTFOUND;
    DBMS_OUTPUT.PUT_LINE('Pledge Amount: ' || v_pledge_amt || ', Pledge Date: ' || v_pledge_date || ', Project Name: ' || v_project_name);
  END LOOP;
  CLOSE pledge_cursor;

  
  DBMS_OUTPUT.PUT_LINE('Payment Information for Donor ID: ' || v_donor_id);
  OPEN payment_cursor;
  LOOP
    FETCH payment_cursor INTO v_pay_amt, v_pay_date, v_pay_method;
    EXIT WHEN payment_cursor%NOTFOUND;
    DBMS_OUTPUT.PUT_LINE('Payment Amount: ' || v_pay_amt || ', Payment Date: ' || v_pay_date || ', Payment Method: ' || v_pay_method);
  END LOOP;
...
DECLARE
  -- Declare a variable to hold the donor's pledge and payment information
  v_donor_id     dd_donor.idDonor%TYPE := 303; -- Sample donor ID for which we want to retrieve information
  v_pledge_amt   dd_pledge.Pledgeamt%TYPE;
  v_pledge_date  dd_pledge.Pledgedate%TYPE;
  v_project_name dd_project.Projname%TYPE;
  v_pay_amt      dd_payment.Payamt%TYPE;
  ...
  -- Declare cursor variables (ref cursors)
  TYPE pledge_cursor_type IS REF CURSOR;
  TYPE payment_cursor_type IS REF CURSOR;
  
  -- Declare cursor variables for pledges and payments
  v_pledge_cursor pledge_cursor_type;
...
BEGIN
  ...
  LOOP
    FETCH v_pledge_cursor INTO v_pledge_amt, v_pledge_date, v_project_name;
    EXIT WHEN v_pledge_cursor%NOTFOUND;
    DBMS_OUTPUT.PUT_LINE('Pledge Amount: ' || v_pledge_amt || ', Pledge Date: ' || v_pledge_date || ', Project Name: ' || v_project_name);
  END LOOP;

  CLOSE v_pledge_cursor;
  ...
  LOOP
    FETCH v_payment_cursor INTO v_pay_amt, v_pay_date, v_pay_method;
