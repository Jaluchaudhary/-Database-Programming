...
CREATE OR REPLACE PROCEDURE DDPROJ_SP (
    ...
    v_Projname dd_project.Projname%TYPE;
    v_Projstartdate dd_project.Projstartdate%TYPE;
    v_Projenddate dd_project.Projenddate%TYPE;
    v_Projfundgoal dd_project.Projfundgoal%TYPE;
    v_ProjCoord dd_project.ProjCoord%TYPE;
BEGIN


    SELECT Projname, Projstartdate, Projenddate, Projfundgoal, ProjCoord
    INTO v_Projname, v_Projstartdate, v_Projenddate, v_Projfundgoal, v_ProjCoord
    FROM dd_project
    WHERE idProj = p_idProj;

  
    DBMS_OUTPUT.PUT_LINE('Project Name: ' || v_Projname);
    DBMS_OUTPUT.PUT_LINE('Start Date: ' || TO_CHAR(v_Projstartdate, 'MM-DD-YYYY'));
    DBMS_OUTPUT.PUT_LINE('End Date: ' || TO_CHAR(v_Projenddate, 'MM-DD-YYYY'));
    DBMS_OUTPUT.PUT_LINE('Fund Goal: ' || v_Projfundgoal);
    DBMS_OUTPUT.PUT_LINE('Project Coordinator: ' || v_ProjCoord);

EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No project found with idProj = ' || p_idProj);
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('An error occurred: ' || SQLERRM);
END DDPROJ_SP;
/

EXEC DDPROJ_SP(p_idProj => 502);


CREATE OR REPLACE PROCEDURE DDPAY_SP (
    p_idDonor IN NUMBER
) IS
    -- Variable to store the count of active pledges
    v_active_pledge_count NUMBER;
BEGIN
    -- Count the active pledges for the donor
    SELECT COUNT(*)
    INTO v_active_pledge_count
    FROM dd_pledge p
    WHERE p.idDonor = p_idDonor
      AND p.paymonths > 0  
      AND p.idStatus NOT IN (20, 40);  

   
    IF v_active_pledge_count > 0 THEN
        DBMS_OUTPUT.PUT_LINE('Donor ' || p_idDonor || ' has an active pledge with monthly payments.');
    ELSE
        DBMS_OUTPUT.PUT_LINE('Donor ' || p_idDonor || ' does not have any active pledge with monthly payments.');
    END IF;
    
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No data found for donor ' || p_idDonor || '.');
    WHEN OTHERS THEN
...
CREATE OR REPLACE PROCEDURE DDCKPAY_SP (
    p_idPledge IN NUMBER,         -- Pledge ID to check the payment for
    p_expectedAmount IN NUMBER     -- Expected monthly payment amount
) IS
    v_actualAmount NUMBER;        -- Variable to hold the actual payment amount
    v_monthsPaid NUMBER;          -- Number of months for the pledge
    v_pledgeAmt NUMBER;           -- The total pledge amount
    v_monthlyPayment NUMBER;      -- The expected monthly payment amount
BEGIN
    -- Retrieve the total pledged amount and the number of months from DD_Pledge
    SELECT Pledgeamt, paymonths
    INTO v_pledgeAmt, v_monthsPaid
    FROM dd_pledge
    WHERE idPledge = p_idPledge;
    
    -- Calculate the expected monthly payment
    v_monthlyPayment := v_pledgeAmt / v_monthsPaid;
    
    -- Retrieve the actual payment amount for the current month
    SELECT SUM(Payamt)
    INTO v_actualAmount
    FROM dd_payment
    WHERE idPledge = p_idPledge
    AND TO_CHAR(Paydate, 'YYYY-MM') = TO_CHAR(SYSDATE, 'YYYY-MM'); 
    

    IF v_actualAmount = p_expectedAmount THEN
        DBMS_OUTPUT.PUT_LINE('Payment is correct.');
    ELSE
        DBMS_OUTPUT.PUT_LINE('Payment is incorrect. Expected: ' || p_expectedAmount || ', but got: ' || v_actualAmount);
    END IF;
    
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No data found for the given pledge ID.');
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('An error occurred: ' || SQLERRM);
END DDCKPAY_SP;
/
