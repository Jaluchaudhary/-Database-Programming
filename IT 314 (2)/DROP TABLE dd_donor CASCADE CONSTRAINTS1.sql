...
-- Create the DD_PAYTRACK table to track pledge payment activity
CREATE TABLE DD_PAYTRACK (
    track_id NUMBER PRIMARY KEY,           -- Primary key for the table
    username VARCHAR2(30),                 -- Username of the user performing the action
    action VARCHAR2(10),                   -- Action type: INSERT, UPDATE, DELETE
    idpay NUMBER(6),                       -- The idpay value of the changed payment record
    action_date DATE DEFAULT SYSDATE,      -- The date when the action was performed
    idPledge NUMBER(5),                    -- The ID of the associated pledge
    TotalPaid NUMBER(10,2),                -- Total paid so far
    AmountRemaining NUMBER(10,2),          -- Remaining amount to be paid
    LastPaymentDate DATE,                  -- Date of the last payment made
    CONSTRAINT paytrack_idPledge_fk FOREIGN KEY (idPledge) 
        REFERENCES dd_pledge (idPledge)   -- Foreign key to the dd_pledge table
);
...
-- Create the DD_PTRACK_SEQ sequence
...
CREATE OR REPLACE TRIGGER trg_dd_paytrack
AFTER INSERT OR UPDATE OR DELETE ON DD_Payment
FOR EACH ROW
BEGIN
    -- Insert a new record into DD_PAYTRACK on INSERT
    IF INSERTING THEN
        INSERT INTO DD_PAYTRACK (track_id, username, action, idpay, idPledge, TotalPaid, AmountRemaining, LastPaymentDate)
        VALUES (DD_PTRACK_SEQ.NEXTVAL, USER, 'INSERT', :NEW.idPay, :NEW.idPledge, :NEW.Payamt, :NEW.Payamt, SYSDATE);
    
    -- Insert a new record into DD_PAYTRACK on DELETE
    ELSIF DELETING THEN
        INSERT INTO DD_PAYTRACK (track_id, username, action, idpay, idPledge, TotalPaid, AmountRemaining, LastPaymentDate)
        VALUES (DD_PTRACK_SEQ.NEXTVAL, USER, 'DELETE', :OLD.idPay, :OLD.idPledge, :OLD.Payamt, :OLD.Payamt, SYSDATE);
    
    -- Insert a new record into DD_PAYTRACK on UPDATE
    ELSIF UPDATING THEN
        INSERT INTO DD_PAYTRACK (track_id, username, action, idpay, idPledge, TotalPaid, AmountRemaining, LastPaymentDate)
        VALUES (DD_PTRACK_SEQ.NEXTVAL, USER, 'UPDATE', :NEW.idPay, :NEW.idPledge, :NEW.Payamt, :NEW.Payamt, SYSDATE);
    END IF;
END;
/


-- Insert a new payment record into DD_Payment
INSERT INTO DD_Payment (idPay, idPledge, Payamt, Paydate, Paymethod)
VALUES (1001, 500, 200.00, SYSDATE, 'CC');

-- Check if the action is logged in DD_PAYTRACK
...
-- Check if the update action is logged in DD_PAYTRACK
SELECT * FROM DD_PAYTRACK;
...
-- Check if the delete action is logged in DD_PAYTRACK
SELECT * FROM DD_PAYTRACK;


-- Example of inserting data into DD_PAYTRACK table
INSERT INTO DD_PAYTRACK (track_id, username, action, idpay, idPledge, TotalPaid, AmountRemaining, LastPaymentDate)
VALUES (DD_PTRACK_SEQ.NEXTVAL, 'admin', 'INSERT', 1001, 500, 200.00, 420.00, SYSDATE);


-- Insert valid data into dd_pledge to satisfy the foreign key constraint
INSERT INTO dd_pledge (idPledge, pledgeAmount, other_columns)
VALUES (500, 1000, 'some_value');  -- Replace 'some_value' and other columns as necessary
