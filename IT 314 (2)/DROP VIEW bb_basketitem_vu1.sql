...
SHOW ERRORS TRIGGER bb_reorder_trg;
...
BEGIN
  IF :NEW.stock <= :NEW.reorder THEN
    -- Check if there are any open orders (orders that haven't been received yet)
    SELECT SUM(qty)
    INTO v_onorder_num
    ...
    -- If no open orders are found, initialize v_onorder_num to 0
    IF v_onorder_num IS NULL THEN
      v_onorder_num := 0;
    END IF;

    -- If no open order, create a new product request
...
FOR EACH ROW
BEGIN
  -- Check if the DTRECD column was updated (i.e., request is marked as fulfilled)
  IF :NEW.dtRecd IS NOT NULL AND :OLD.dtRecd IS NULL THEN
...
SET dtRecd = SYSDATE, cost = 100 -- Set the received date and cost (adjust as needed)
WHERE idRequest = 3;
COMMIT;
...
FOR EACH ROW
BEGIN
  -- If the request is marked as fulfilled (DTRECD is not null)
  IF :NEW.dtRecd IS NOT NULL AND :OLD.dtRecd IS NULL THEN
    -- Update the stock in the product table (increase stock)
    UPDATE bb_product
    SET stock = stock + :NEW.qty
    WHERE idProduct = :NEW.idProduct;
  
  -- If the fulfillment is canceled (DTRECD is set to null)
  ELSIF :NEW.dtRecd IS NULL AND :OLD.dtRecd IS NOT NULL THEN
    -- Update the stock in the product table (decrease stock)
    UPDATE bb_product
...
VALUES (4, 5, SYSDATE, 45, TO_DATE('15-JUN-2012', 'DD-MON-YYYY'), 225);
COMMIT;
...
INSERT INTO bb_product_request (idRequest, idProduct, dtRequest, qty, dtRecd, cost, idVender)
VALUES (1, 4, TO_DATE('27/11/2024', 'DD/MM/YYYY'), 25, NULL, 25, 123);
COMMIT;


SELECT idRequest, idProduct, TO_CHAR(dtRequest, 'DD/MM/YYYY') AS dtRequest, dtRecd, cost, qty, idVender
FROM bb_product_request
WHERE idRequest = 1;
